...
# Respuestas base para evitar errores 404/500 de redirección en Vercel
@app.get("/")
def read_root():
    return {"message": "Sistemas de Ultron en línea. Servidor FastAPI operando correctamente."}
...
# Herramienta externa de búsqueda en Google
...
# Ruta para transcripción de audio (Requiere python-multipart en requirements.txt)
@app.post("/api/listen")
...
# Ruta principal de procesamiento de texto, toma de decisiones y respuesta de voz
@app.post("/api/think")
async def think_and_speak(request: ChatRequest):
    try:
        ...
        system_prompt = (
            ...
            "Tienes incorporada la herramienta 'code_interpreter' para ejecutar código Python de forma nativa. "
            "Si te piden datos o búsquedas en tiempo real, usa 'buscar_en_google'. Sé directo, conciso y majestuoso."
        )
        ...
        if msg.tool_calls:
            ...
            if tool_call.function and tool_call.function.name == "buscar_en_google":
                ...
                final_response = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "Eres Ultron. Comunica el resultado obtenido imponentemente."},
        ...
        # Generación de la respuesta de voz con OpenAI TTS
